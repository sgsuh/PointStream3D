...
#### 8.3.1 페이지가 떠난 뒤 남는 죽은 포트 (2026-08-15 수정) — 측정 로그

§8.3의 생명주기 처리는 **SW가 죽는 방향**만 다뤘다. 반대 방향에 구멍이 있었다: **페이지가 죽고 SW가 사는 경우.**

워커는 페이지 소유라 페이지와 함께 죽지만 SW는 살아남아 그 `MessagePort`들을 계속 들고 있다. 죽은 워커로 가는 포트는 `postMessage`를 조용히 받아들이고 **영원히 답하지 않는다.** 그래서 그 포트로 배정된 타일은 `DECODE_TIMEOUT_MS`(60초)를 다 기다린 뒤에야 인라인으로 폴백한다. 포트를 회수하는 경로는 `COPCPointCloud.destroy()` → `RELEASE`뿐인데 리로드는 그걸 부르지 않는다 — 즉 **새로고침 한 번이 죽은 포트 6개를 남긴다.**

콘솔에서는 `decode worker failed, falling back: decode worker did not respond`가 워커 수만큼 반복되는 모습으로 나타난다. 데모를 새로고침하는 것만으로 재현되므로 사용자가 가장 밟기 쉬운 경로였다.

두 겹으로 막았다. 둘 다 `scripts/smoke-reload.mjs`가 검증한다.

1. **페이지가 `pagehide`에서 포트를 반납한다**(`src/decodePool.ts`). `unload`가 아니라 `pagehide`인 이유는 모바일에서도 발화하고 bfcache 자격을 잃지 않기 때문이다. bfcache에서 복원된 페이지는 워커는 살아 있고 등록된 포트만 없는데, SW가 빈 풀을 보고 `need-ports`를 보내므로 스스로 복구된다.
2. **SW가 사라진 클라이언트의 포트를 스스로 걷어낸다**(`src/sw/sw.ts`). 새 풀이 핸드오버될 때 `clients.matchAll()`에 없는 `clientId`의 포트를 버린다 — (1)의 메시지가 전달되지 못한 경우에 남는 유일한 방어선.

부수적으로 **풀 id를 페이지 로컬 카운터에서 uuid로 바꿨다.** `pool-1`이 모든 탭에서 재발급되던 탓에, 한 탭의 `RELEASE`가 다른 탭의 포트를 떨어뜨릴 수 있었다. (1)을 넣으면 리로드마다 그 오작동이 일어나므로 함께 고쳐야 했다.

수정 전/후, autzen 기본 카메라(33 타일), workers=6:

| 시나리오 | | 수정 전 | 수정 후 |
|---|---|---:|---:|
| **리로드** | 로드 시간 | 62.2 s | **6.0 s** |
| | `fallbacks` | 12건 | **0건** |
| | 풀 디코드 지연 합 | 730,391 ms | **14,140 ms** |
| **포트 미반납 탭 종료** | 로드 시간 | 62.2 s | **6.6 s** |
| | `fallbacks` | 6건 | **0건** |

> 재현에서 걸린 두 가지. ① `poolSize`는 판별 기준이 못 된다 — 타임아웃이 이미 죽은 포트를 떨어뜨린 뒤라 사후에는 정상값으로 보인다. **로드 소요 시간과 `fallbacks`로 판정해야 한다.** ② **탭 크래시로는 재현되지 않는다** — 같은 사이트 탭들이 렌더러 프로세스를 공유해 살아남을 탭까지 죽고, 클라이언트가 0이 되면 브라우저가 SW를 은퇴시켜 포트가 애초에 안 남는다. (2)를 검증하려면 다른 탭을 열어 SW를 살려둔 채 `pagehide` 등록만 막아야 한다.
